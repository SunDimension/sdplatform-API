...
class SalesOrderController extends Controller
{
    ...
    // {
    //     $salesOrder = SalesOrder::with(['itemSold.product', 'itemSold.store', 'itemSold.measurement', 'customer', 'creditTransaction'])
    //         ->where('sales_order_number', $orderno)
    //         ->withSum('salesReceipts as total_paid', 'amount_paid')
    //         ->firstOrFail();
    ...
    //     // Get approved inflows only (status 3)
    //     $inflows = PostInflow::where('customer_id', $customerId)
    //         ->where('inflow_status', 3)
    //         ->sum('amount');

    //     // Get all outflows for this customer
    //     $outflows = PostOutflow::where('customer_id', $customerId)
    //         ->sum('amount');
    ...
    //     return response()->json([
    //         'data' => new SalesOrderResource($salesOrder, $balance)
    //     ]);
    // }
